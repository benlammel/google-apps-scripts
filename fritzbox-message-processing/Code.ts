function getGmailData() {
  ...
  let unreadThreads: GoogleAppsScript.Gmail.GmailThread[] = GmailApp.search('(subject:("FRITZ!Box info: Data on usage and connections" OR "FRITZ!Box-Info: Nutzungs- und Verbindungsdaten") AND NOT label:fritzbox.usage.processed)');
  ...
  if (unreadThreads.length > 0) {
    ...
    unreadThreads.forEach((threadItem: GoogleAppsScript.Gmail.GmailThread) => {
      threadItem.getMessages().forEach((msgItem: GoogleAppsScript.Gmail.GmailMessage) => {
        if(!msgItem.getPlainBody().includes('Begin forwarded message')){
